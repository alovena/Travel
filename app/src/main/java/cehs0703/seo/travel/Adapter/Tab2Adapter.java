...
public class Tab2Adapter extends BaseAdapter {
    ...
    ArrayList<Tab2ItemVO> list=new ArrayList<>();

    public Tab2Adapter(ArrayList<Tab2ItemVO> list) {
        this.list = list;
    }
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            ...
            convertView = inflater.inflate(R.layout.fragment_tab2_list, parent, false);
        }

        // 화면에 표시될 View(Layout이 inflate된)으로부터 위젯에 대한 참조 획득
        ImageView iconImageView = (ImageView) convertView.findViewById(R.id.Tab_Image2) ;
        TextView titleTextView = (TextView) convertView.findViewById(R.id.TabFrag_Text2) ;
        // Data Set(listViewItemList)에서 position에 위치한 데이터 참조 획득


        Tab2ItemVO listViewItem = list.get(position);
    ...
    public void addVO(String icon, String title) {
        //Tab2ItemVO item = new Tab2ItemVO();
        //item.setImg(icon);
        //item.setName(title);
        //list.add(item);
     }
